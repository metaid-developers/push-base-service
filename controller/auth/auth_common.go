...
func SignTextMessage2(message, privateKey string) (string, error) {
	// Decode a hex-encoded private key.
	pkBytes, err := hex.DecodeString(privateKey)
	if err != nil {
		return "", err
...
func VerifyTextSign2(message, messageSign, publicKey string) (bool, error) {
	sigBytes, err := base64.StdEncoding.DecodeString(messageSign)
	if err != nil {
		return false, err
	}

	expectedMessageHash := chainhash.HashB([]byte(messageSignatureHeader + message))
	fmt.Printf("expectedMessageHash:%s\n", hex.EncodeToString(expectedMessageHash))
	pk, compressed, err := ecdsa.RecoverCompact(sigBytes,
		expectedMessageHash)
	if err != nil {
		return false, err
	}
